...
public class DBTaskCrawler extends Crawler{
    public static final String FROM_QUEUE="qunaer_product_db";
    ...
    private void saveData(String taskData){
        Bean bean=gson.fromJson(CodeUtil.unicodeToChinese(taskData),Bean.class);
        ...
        try{
            ...
            if(!BFUtil.isExist(bean.productLink)){
                preparedStatement=connection
                        .prepareStatement("INSERT INTO QUNAER_TRAVEL_PRODUCT_INFO(" +
                                "`PROD_UNI_CODE`,`OTA_ID`,`PROD_TYPE`,`OTA_PROD_ID`,`PROD_NAME`," +
                                "`TA_NAME`,`PACKAGE_NAME`,`TOUR_DAYS`,`TRAVEL_PLAN`,`PROD_LINK`)" +
                                "VALUES(?,?,?,?,?,?,?,?,?,?);");
                ...
                preparedStatement.setString(8,bean.duration);
                preparedStatement.setString(9,bPackage.path);
                preparedStatement.setString(10,bean.productLink);
                ...
                preparedStatement=connection.prepareStatement("SELECT PROD_UNI_CODE FROM " +
                        " QUNAER_TRAVEL_PRODUCT_INFO WHERE PROD_LINK=?");
            ...
            preparedStatement=connection.prepareStatement("INSERT INTO QUNAER_TRAVEL_PRODUCT_PRICE(" +
